...
Set a prediction threshold and examine sensitivity and specificity (false-positives) below. Find the best cutoff level. AUC calculated using the trapezoidal rule at the cutoff level. The threshold that minimizes mis-classification is where the sum of the number of false-positive and false-negative results (error rate) is lowest.
...
For example, a net benefit of 0.0973 indicates that the difference between our true-positives and false-positives will be finding about 10 more true-positives than false-positives per 100 patients using a cutoff of FEV= 3.
...
This adds a little more detail. We see across all of the key percentiles, this model outperforms a potentially costly approach of "treat all" or "treat none". 
...
As we expect, a model can inform us which patients to treat as well as which patients we can avoid treatment. This is especially helpful if treatments have permanent side effects such as taking biopsies or when working with less critical outcomes, we can save money by administering treatments that may be unnecessary for most patients.
