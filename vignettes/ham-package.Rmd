...
* y= "roll" is the 'rolling' average (e.g., rolling 12-month average).
...
Here are OLS and logistic regression examples using assess() on the mtcars data. This ogistic regression model code not run:
...
### ham provides coefficient interpretations with interpret()
```{r ols_interpret}
# Model summary
interpret(assess(hp ~ mpg+wt, data=mtcars, regression="ols"))$model
```

In this example, we predict the binary outcome of Automatic or Manual car transmission.
...
In this example, we examine the Hospital Acquired Infection rate over 3+ years. Notice the offset(log(PatientDays)) part of the formula, offsets are common in health care analyses and allow to take into account varying exposures like patient days or can be used for observed/expected ratios, e.g., offset(log(Expected)). Another option is to log transform your exposure variable as a variable in your data frame and enter it into the offset argument. Please note that in assess, the offset argument uses a character name, thus offset= 'logPatientDays'.
...
We see that there is a significant Incidence Rate by month as seen in the interpretations below.

### Poisson regression interpretations
```{r pm1_interpret}
# Specify how many significant digits with digits argument
...
Descriptive statistics on newly created variables and the original cost as a comparison. top.cost is the topcoded cost variable. R^2 is not bad.
```{r summary topcoding propensity}
summary(m1$newdata[, c( "cost","top.cost", "pscore")])
```

In the previous example, we used covariate adjustment with the propensity score but now we will use inverse probability of treatment weighting using the propensity score (IPW). There is a weights argument that we can include a character vector for 3 types of IPW weights: "ipw" (Horvitz-Thompson estimator), "nipw" (Normalized IPW by Hajek estimator), and "att". Trimming of propensity scores is also available with 'trim' argument. See assess() documentation to read more. 

### Inverse Probability of Treatment Weighting using the propensity score (IPW)
```{r ols ipw weights}
...
### Weighted least squares model results
```{r ols ipw results}
summary(ipw1$model)
```
...
This graph will be added in version 1.3 and it is a way to get summary of the coefficients, showing the direction of the effect with point estimates and 95% confidence intervals. This was inspired by Frank Harrell's plot.summary in the rms package. Instead ham uses plot.Summary (upper case 'S') with the almost identical summary (lower case 's') but adds some class information to help with plotting. This will work on assess regression objects for OLS linear, logistic, Poisson and models coming from Base R's lm() and glm() as well as the survival package and possibly other glm() models. It can work with Cox Proportional Hazards from the survival package. And this will work with the differences-in-differences and interrupted time series models created with assess().

OLS regression--most basic format with Base R lm()
```{r plotSummary1, fig.dim = c(6, 4.5)}
m01 <- lm(mpg ~ wt+hp+am, data=mtcars)
# Most basic code, note it uses Summary() with a captialized 'S'
plot(Summary(m01))
```

Take a look at the effect when hp is increased by 83.5

```{r Summary model 2 results}
m02 <- assess(formula=mpg ~ wt+hp+am, data=mtcars, regression="ols")
# Using the assess function, notice 'm02$model' object below
print(Summary(m02$model), digits=4)
```

let's see the impactful 83.5 hp increase in going from the 1st to 3rd quartiles
```{r plotSummary2, fig.dim = c(6, 4.5)}
m02 <- assess(formula=mpg ~ wt+hp+am, data=mtcars, regression="ols")
plot(x=Summary(m02$model, increase=c(hp= 83.5)))
```

Logistic model
```{r plotSummary3, fig.dim = c(6, 4.5)}
m03 <- glm(vs ~ wt+hp+am, data=mtcars, family="binomial")
# Display options, sorted plot, and printing the 95% CIs to review
plot(x=Summary(m03, increase=c(wt=1.1375, hp= 83.5) ), color="aquamarine", lwd=5,
...
```{r plotSummary4, fig.dim = c(6, 4.5)}
...
Now lets graph the results with the counterfactual (what could have happened without the intervention program). 
...
You've probably noticed only 1 trend line because we only have 1 group, the intervention group. In the 1 group models, our counterfactual lines appear to continue from the end of the baseline. However, we could continue the line from the the second period when there are 2 interruptions.
...
Let's take a look at the interpretations.

### interpret ITS 2 coefficients
```{r its model 2 interpretations}
interpret(im12)$its
```
...
### Plot the summary coefficients

plot(Summary(x)) also works for ITS and DID causal models through ham
```{r plotSummary5, fig.dim = c(6, 4.5)}
im22 <- assess(formula=los ~ ., data=hosprog, intervention = "program",
int.time="month", interrupt= c(5, 9), its="two")
# The intervention group had the biggest change between the baseline and month 5
plot(Summary(im22$ITS), sort="coef", decreasing=TRUE, main="ITS summary", color="red", pcol="green", lwd=2, pt.cex=1.5)
```
...
<span style="color: red;" >Hint:</span> The 2 summaries at the bottom help us to understand the overall effect on the LOS reduction up to the end of those time periods. In other words, the coefficient interpretations explain what each means while the summaries give us an idea that each value represents the change per unit of time in that period, the degree of lower (or higher) LOS over time for the intervention and/or control groups. For example, the intervention group saw a slight decrease of 0.037 per month after the first interruption but saw an increase of about 0.192 per month after the second interruption.
...
### ITS model 4: 30-day death (mortality)
```{r its model 4}
...
### interpret ITS 4 coefficients
```{r its model 4 interpretations}
interpret(id22)$its
```

Let's take a look at the graph (intervention is brown, control is gray).

### Graph ITS model 4 
```{r plotITS04, fig.dim = c(6, 4.5)}
...
In the latest version of ham, we now have the option of an interrupted time series that tests as many interruptions as the data permits. This example demonstrates the US unemployment rate since 1929, highlighting key moments during that period. 
...
We see that many things increased or decreased unemployment rates. The biggest shift after the stock market crash in unemployment seems to be FDR's New Deal.

### Graph ITS model 5 results
