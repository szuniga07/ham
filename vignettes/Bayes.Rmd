...
* General overview of the MCMC representativeness, accuracy, and efficiency.

* Gelman-Rubin, ACF, ESS, and MCSE statistics about your parameter.
...
# 4b. Variation analysis: 'vary'

This option, ‘vary’, is a modified version of 'check' and allows us to view multiple posterior predictive check lines. The primary purpose is for use in a Bayesian differences-in-differences study on the variation parameter. For example, in a model with a normal distribution in the likelihood function, we would compare the change in the standard deviation for the treatment and control groups. Or if we wanted to compare the Great Lakes' rainfall and evaporation rates, we can produce posterior predictive checks for Ontario, Superior, etc.

The motivation for ‘vary’ is to test if the treatment group reduces variation. An important goal in hospital quality improvement is to reduce variation in our health outcomes. And a Bayesian DID can be done by estimating the parameters for the control group’s pre- and post-intervention values and the treatment group’s pre- and post-intervention values and then mathematically calculate the DID coefficient.

For example, we could calculate the DID for variation in a Bayesian model:

#Control group (post intervention - pre intervention)

  delta21SigmaOfY <- sigmaOfY[2] - sigmaOfY[1]
  
#Treatment group (post intervention - pre intervention)

  delta43SigmaOfY <- sigmaOfY[4] - sigmaOfY[3]
  
#DID for 'variation' 

B3DIDSigma <- delta43SigmaOfY - delta21SigmaOfY 

We can also use ‘vary’ for the DID of the mean or if we just want to view multiple posterior predictive checks if we select ‘type’ as 'n', 'ln', 'sn', 'w', 'g', or 't'. 

Below, we have code to do a variation analysis of a model with a log-normal distribution for length of stay (LOS). The losvary object requires we modify the 
hosprog data by adding the predictor variable for 'Post.Int'. 
...
We can see that the post-intervention period for the treatment group has a lower estimated length of stay when compared to the pre-intervention period.

Because a log-normal model was utilized, we get DID estimates for the mean, median, and mode. For completeness, we'll review the DID on average reductions in LOS for the treatment group.

Mean DID

```{r plotDIDavg1, fig.dim = c(6, 4.5)}
plot(x=bvlos, y="post", parameter="B3DIDMu", bcol="yellow",
     HDItext=.3, cex=2, main= "DID for the mean")
```

Median DID

```{r plotDIDavg2, fig.dim = c(6, 4.5)}
plot(x=bvlos, y="post", parameter="B3DIDMed", bcol="tomato",
     HDItext=.3, cex=2, main= "DID for the median")
```

Mode DID

```{r plotDIDavg3, fig.dim = c(6, 4.5)}
plot(x=bvlos, y="post", parameter="B3DIDMode", bcol="slategray",
     HDItext=.3, cex=2, main= "DID for the mode")
```

Either way you look at it, the average DID showed a significant improvement for the treatment group.


# 4c. Checking the regression trend line: 'check'
...
# 5. Hierarchical or Multilevel Model summary with multilevel/shrinkage plots: 'multi' 
...
lcol="blue", pcol= c("red", "skyblue"), round.c=2, bcol="yellow",
xlim=c(-.1, 1), legend=NULL, add.legend="topright", lwd=3, cex.lab=1.2,
cex= 2, cex.main=1.25, cex.axis=.75, cex.legend=1.5, xlab=NULL)
```
...
round.c=3, bcol="lavender", xlim=c(-.1, 1), legend=NULL, add.legend="bottomleft", lwd=3, cex.lab =1.2, cex= 2, cex.main=1.25, cex.axis=.75, cex.legend=1.25, xlab=NULL)
```
...
We can review these results better by getting the interpretations.
...
For example, the raw difference of 0.10 between potential targets of 0.50 and 0.40 results in a Cohen's h effect size of 0.20, representing a small effect size. Some stakeholders may argue that the alternative target of 0.40 is too big of change while others may argue it is too small (i.e., context is important). If we have an intervention program that is well documented in reducing length of stay, setting a target that would represent a small effect size could be a good option. But if we have no programs in place to improve length of stay, setting targets at a level that is less than a small effect, could be a fair and realistic option. For the former option, set the target to around the 40th percentile at 3.92 and for the latter option, set the target to around the 45th percentile at 4.18, based on the interpretations. 

For the administrators who like "round numbers", we see in our output that setting a target of 4 days would represent a Cohen's h effect size that would be less than a small effect so this mayb be a helpful alternative. Another thing to consider is how difficult some outcomes are to move. Cohen's effect sizes are based on "average" types of situations so we should consider that as well as how difficult it is to improve outcomes and how effective the programs (or predictors) are. 

Notice that in the interpretations, the estimated mean of the X values from the beta distribution has NA. Because this model didn't use a beta distribution likelihood (i.e., type="n"), this estimate doesn't apply to these target statistics.

Here is the bare minimum code that will run (if we want to print, newdata=TRUE is needed; displaying default)
btarget1 <- Bayes(x=losmcmc, y="target", type="n", parameter=c("muOfY","sigmaOfY"),
