...
MCMC chains created in the coda package (e.g., through JAGS or Stan) should also be easily converted with the Bayes function.
...
* multi: Creates a posterior summary of a hierarchical model.
...
* multi: Graphs a posterior summary of a hierarchical model with 2 options of repeats measurements nested within individuals or individuals nested within organizations. For example, multiple blood tests for patients found within hospitals.

* target: Plot the calculated the proportion of the distribution above specific probabilities or values using either the parameter estimate that represents the 'average' (i.e., graph from 'post') or the estimated distribution (i.e., graph from 'check').
...
Review autocorrelation factor, density plots, Gelman-Rubin, and traceplots statistic.
...
On how well our model fits the data. Estimating center and spread for hospital length of stay. Generally, we only need the first 6 arguments but we'll modify the plot. A model with a gamma likelihood would fit better but this will do for now.
...
And now the level 3 plot (observation in plants in Treatment by type groups). You'll notice that in both the graphs, there is a wide HDI band, partially due to only having a few groups at each level, here just 4 groups. Having more groups can help narrow our degree of uncertainty by providing more information.
...
Our administrators ask how far are we from our goals, they ask about future targets in increments of 5 points of probability or specific days. We answer both. 'newdata' for plots. Below is the full set of results for each target.
...
Target graph using a posterior predictive check, more intuitive. T
